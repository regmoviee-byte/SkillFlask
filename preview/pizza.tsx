// Dev-only preview of the pizza theme: `npx vite` serves it at /preview/pizza.html.
// URL params: ?dark, ?tg=purple (purple Telegram theme), ?motion=reduced, ?levels=N.
...
import { pizzaTheme } from '../src/ui/progress/themes/pizza';
...
const { Hero, Mini, text } = pizzaTheme;
...
// The edge cases: four long captions, marks at the very start and end, two close together.
const MARKS4: ProgressMark[] = [
  { id: 'e1', label: 'Старт', height: 0 },
  { id: 'e2', label: 'Первая глава учебника', height: 0.05 },
  { id: 'e3', label: 'Контрольная работа по теме', height: 0.1 },
  { id: 'e4', label: 'Конец', height: 1 },
];
...
function Preview() {
  ...
  const levelUp = async () => {
    ...
    setBusy(true);
    setStageFill(0.2);
    await stage.current?.playLevelUp({ fromFill: 0.9, toFill: 0.2, levels });
    setBusy(false);
  };
  ...
  return (
    <>
      ...
      <div className="cell" id="marks4">
        <Hero fill={0.06} capacity={10000} motion={motion} marks={MARKS4} onMarkTap={(id) => setTapped(`tapped ${id}`)} />
        <div className="meta">fill 0.06, 4 marks, capacity 10 000</div>
      </div>
      <div className="cell">
        <Hero fill={0.4} capacity={1200} motion={motion} />
        <div className="meta">capacity 1 200</div>
      </div>
